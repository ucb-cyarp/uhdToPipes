...
void* txHandler(void* argsUncast) {
    ...
    UHD_API status = uhd_tx_streamer_max_num_samps(tx_streamer, &samps_per_buff);
    if(status){
    ...
    while(running) {
        ...
        for(int block = 0; block < numTransmissions; block++){
            ...
            UHD_API status = uhd_tx_streamer_send(tx_streamer, buffs_ptr, samps_per_buff, &tx_md, 10, &num_samps_sent);
            if(status){
                running = false; //not actually needed
        ...
        if(forceFullTxBuffer){
            ...
            size_t num_samps_sent = 0;
            UHD_API status = uhd_tx_streamer_send(tx_streamer, buffs_ptr, sampsReamining, &tx_md, 10, &num_samps_sent);
            if(status){
                running = false; //not actually needed
                *terminateStatus = true;
                printf("Error sending to USRP\n");
